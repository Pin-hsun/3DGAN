...
class l1GradientLoss():
    ...
    def getloss(self, fakeIm):
        ...
        for i in range(self.n_scale):
            fakeIm = self.downsample(fakeIm)
            grad_fx, grad_fy = self.grad_xy(fakeIm)
            loss += self.criterion(grad_fx, torch.zeros_like(grad_fx)) + self.criterion(grad_fy,
                                                                                        torch.zeros_like(grad_fy))
...
if __name__ == '__main__':
    real_B_encoded = torch.rand(2, 1, 256, 256)
    enc_c = E_content(input_dim_a=1, input_dim_b=1)
    enc_acat = E_attr_concat(input_dim_b=1, output_nc=8, norm_layer=None,
                                                nl_layer=get_non_linearity(layer_type='lrelu'))

    mu_b, logvar_b = enc_acat(real_B_encoded)
    std_b = logvar_b.mul(0.5).exp_()
    eps_b = get_z_random(std_b.size(0), std_b.size(1), 'gauss')
    z_attr_b = eps_b.mul(std_b).add_(mu_b)
