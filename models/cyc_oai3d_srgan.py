...
from networks.losses_srgan import VGGLoss

def threed_2D(img):
    return torch.cat([img[i, :, :, :, :] for i in range(img.shape[0])], 3).permute(3, 0, 1, 2)
...
class GAN(BaseModel):
    ...
    def __init__(self, hparams, train_loader, test_loader, checkpoints, resume_ep):
        ...
        self.criterion_VGG = VGGLoss(net_type='vgg19', layer='relu2_2')
    ...
    def generation(self, batch):
        ...
        # xy = threed_2D(ori) # (Z*B, C, X, Y)

        self.oriX = xy

        self.imgYX = self.net_g(ori)['out0']  # (B, C, X, Y, Z)
        # self.imgYX_2d = threed_2D(self.imgYX) # (Z*B, C, X, Y)
    ...
    def backward_g(self):
        ...
        # loss_g_l1 = self.add_loss_l1(a=self.imgYX[:, :, :, :, ::8], b=self.ori[:, :, :, :, :])

        # Perception Loss
        vgg_loss = self.criterion_VGG(self.imgYX.permute(4, 1, 2, 3, 0)[::8, :, :, :, 0], self.oriX[::8, :, :, :])
        ...
        loss_g = loss_g_gan + self.hparams.lamb * vgg_loss

        return {'sum': loss_g, 'loss_g_gan': loss_g_gan, 'vgg_loss': vgg_loss}
