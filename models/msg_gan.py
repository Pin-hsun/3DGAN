...
class GAN(BaseModel):
    ...
    def generation(self, batch):
        ...
        self.images_true = [xy] + [avg_pool2d(xy, int(np.power(2, i))) for i in range(1, self.depth)]
        self.images_true = list(reversed(self.images_true))
        self.images_fake = [xy] + [avg_pool2d(self.imgYX, int(np.power(2, i))) for i in range(1, self.depth)]
        self.images_fake = list(reversed(self.images_fake))
