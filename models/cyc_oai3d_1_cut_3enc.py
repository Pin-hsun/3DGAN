...
class GAN(BaseModel):
    ...
    def __init__(self, hparams, train_loader, test_loader, checkpoints, resume_ep):
        ...
        self.net_g1 = copy.deepcopy(self.net_g)
        self.net_g2 = copy.deepcopy(self.net_g)
        ...
        feature_shapes = [32, 64, 128, 256] #ed023d
        # feature_shapes = [64, 128, 128, 128] #resnet2G3D
        ...
        for nce_layer in range(4):#self.nce_layers:
            ...
            # save model names
        self.netg_names = {'net_g': 'net_g', 'net_g1': 'net_g1', 'net_g2': 'net_g2', 'netF': 'netF'}
    ...
    def generation(self, batch):
        ...
        self.feat_q = self.net_g(self.oriup, method='encode')
        self.feat_q_yz = self.net_g(self.oriup, method='encode', direction='yz')
        self.feat_q_xz = self.net_g(self.oriup, method='encode', direction='xz')
        # input = self.feat_q[-1]
        input = torch.max(torch.cat([self.feat_q[-1], self.feat_q_yz[-1], self.feat_q_xz[-1]]), 0)[0].unsqueeze(0)
        # input = torch.cat([self.feat_q[-1], self.feat_q_yz[-1], self.feat_q_xz[-1]], 1)
        self.imgYX = self.net_g(input, method='decode')['out0']
        # print(self.imgYX.shape)
    ...
    def backward_g(self):
        ...
        # temperal loss
        loss_t = self.add_loss_l1(a=temperal_diff(self.imgYX.permute(4, 1, 2, 3, 0)[:, :, :, :, 0]),
        ...
        # feat_q_yz = self.net_g(self.oriup, method='encode', direction='yz')
        feat_k_yz = self.net_g(self.imgYX, method='encode', direction='yz')
        # feat_q_xz = self.net_g(self.oriup, method='encode', direction='xz')
        feat_k_xz = self.net_g(self.imgYX, method='encode', direction='xz')

        feat_k_pool, sample_ids = self.netF(feat_k, self.hparams.num_patches, None)  # get source patches by random id
        feat_q_pool, _ = self.netF(self.feat_q, self.hparams.num_patches, sample_ids)  # use the ids for query target
        feat_k_pool_yz, _ = self.netF(feat_k_yz, self.hparams.num_patches, sample_ids)
        feat_q_pool_yz, _ = self.netF(self.feat_q_yz, self.hparams.num_patches, sample_ids)
        feat_k_pool_xz, _ = self.netF(feat_k_xz, self.hparams.num_patches, sample_ids)
        feat_q_pool_xz, _ = self.netF(self.feat_q_xz, self.hparams.num_patches, sample_ids)

        total_nce_loss = 0.0
        # nce(enc(xy), enc1(yz))
        for f_q, f_k, crit in zip(feat_q_pool, feat_q_pool_yz, self.criterionNCE):
            loss = crit(f_q, f_k) * self.hparams.lambda_NCE
            total_nce_loss += loss.mean()
        # nce(enc(xy), enc2(xz))
        for f_q, f_k, crit in zip(feat_q_pool, feat_q_pool_xz, self.criterionNCE):
            loss = crit(f_q, f_k) * self.hparams.lambda_NCE
            total_nce_loss += loss.mean()
        # nce(enc(out), enc1(out))
        for f_q, f_k, crit in zip(feat_k_pool, feat_k_pool_yz, self.criterionNCE):
            loss = crit(f_q, f_k) * self.hparams.lambda_NCE
            total_nce_loss += loss.mean()
        # nce(enc(out), enc2(out))
        for f_q, f_k, crit in zip(feat_k_pool, feat_k_pool_xz, self.criterionNCE):
            loss = crit(f_q, f_k) * self.hparams.lambda_NCE
            total_nce_loss += loss.mean()

        loss_nce = total_nce_loss / (4 * 4)

        loss_g = loss_g_gan + loss_g_l1 + loss_nce + loss_t

        return {'sum': loss_g, 'loss_g_gan': loss_g_gan, 'loss_g_l1': loss_g_l1, 'loss_nce': loss_nce, 'loss_t': loss_t}
