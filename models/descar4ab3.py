...
class GAN(BaseModel):
    def __init__(self, hparams, train_loader, test_loader, checkpoints):
        ...
        self.netd_names = {'net_d': 'net_d', 'classifier': 'classifier', 'net_dX': 'net_dX'}
    ...
    def test_method(net_g, img, args, a=None):
        ...
        out = net_g(oriY, a=None)
        imgYX = out['out1']
        imgYX = nn.Sigmoid()(imgYX)  # mask

        combinedXY = combine(imgXY, oriX, method='mul')
        combinedXX = combine(imgYX, oriY, method='mul')

        return {'imgXY': imgXY[0, ::].detach().cpu(),
                'imgXX': imgYX[0, ::].detach().cpu(),
    ...
    def generation(self, batch):
        ...
        self.imgYX = nn.Sigmoid()(self.net_g(self.oriY, a=None)['out1'])  # mask
    ...
    def backward_g(self):
        ...
        # ADV(YX)+
        ayx = self.add_loss_adv(a=self.imgYX, net_d=self.net_dX, truth=True)
        ...
        if self.hparams.lbx > 0:
            loss_l1x = self.add_loss_l1(a=self.imgYX, b=self.oriX)
    ...
    def backward_d(self):
        ...
        # ADV(XX)-
        axx = self.add_loss_adv(a=self.imgYX.detach(), net_d=self.net_dX, truth=False)
...
# CUDA_VISIBLE_DEVICES=0,1,2,3 python train.py --jsn womac3 --prj descar4ab3/L100_100/ --dataset womac4 --models descar4ab3  --nm 11 --netD bpatch_16 --split a  --lbvgg 0 --mc --direction ap_bp --lbx 100 --lamb 100 --env a6k --netG dsmcatt