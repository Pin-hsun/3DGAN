...
if __name__ == '__main__':
    enc_a = E_attr_concat(3, 8,
                  norm_layer=None,
                  nl_layer=get_non_linearity(layer_type='lrelu'))

    mu_b, logvar_b = enc_a(torch.rand(10, 3, 256, 256))
    kl = mu_b.pow(2).add_(logvar_b.exp()).mul_(-1).add_(1).add_(logvar_b)
    loss = torch.sum(kl).mul_(-0.5) * 0.01